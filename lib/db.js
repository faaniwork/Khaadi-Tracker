...
export async function listChatMessages(limit = 200) {
  const rows = await queryAll(
    'SELECT id, email, name, text, at, image_id, image_name FROM chat_messages ORDER BY id DESC LIMIT ?',
...
export async function postChatMessage({ email, name, text, imageId, imageName }) {
  ...
  const at = Date.now();
  const result = await d1(
    'INSERT INTO chat_messages (email, name, text, at, image_id, image_name) VALUES (?, ?, ?, ?, ?, ?)',
    [email, name || '', clean, at, imageId || null, imageName || null]
  );
  const id = result?.meta?.last_row_id ?? null;
  return { id, email, name: name || '', text: clean, at, imageId: imageId || null, imageName: imageName || null };
}

/**
 * Deletes one message - your own, always, or anyone's if you're an admin
 * moderating the room. The image itself is left in Drive untouched (someone
 * else may have already opened or saved it); this only removes the message
 * that pointed to it.
 */
