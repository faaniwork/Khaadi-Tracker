...
// 'feedback' sits between approved and rejected: not a rejection, but not
// signed off either. It exists so "needs a small tweak" has somewhere to
// live that isn't a rejection reason, which is what people were overloading
// "other" to mean before this existed.
const REVIEW_STATUSES = ['pending', 'approved', 'feedback', 'rejected'];
...
export function validateReviewDecision({ status, reason, text }) {
  ...
  if (status === 'rejected') {
    ...
    // No reason enum for feedback — it is a free note, not a categorised
    // rejection — but a feedback marker with no text tells nobody anything.
    normReason = null;
    if (!normText) throw new Error('Feedback needs a short note about what to change.');
  } else {
    // An approval or a return to pending carries no rejection reason, so
    // clear any leftover from a previous reject/feedback on the same file.
    normReason = null;
    normText = '';
  }
...
// message from either side survives, in order. Setting a 'feedback' decision
// also drops its note in here (see the /api/drive/review route), so the
// thread reads as one continuous conversation rather than two.
