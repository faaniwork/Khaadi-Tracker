...
export async function applyDriveResync({ email, release, seen, by }) {
  ...
  for (const row of existing) {
    ...
    const key = `${row.collection || ''}␟${row.dress || ''}`;
    if (!orphansByName.has(key)) orphansByName.set(key, []);
    orphansByName.get(key).push(row);
  }
  ...
  for (const folder of seen) {
    ...
    if (!prev) {
      ...
      const key = `${folder.collection}␟${folder.dress}`;
      const candidates = (orphansByName.get(key) || []).filter((r) => !repairedIds.has(String(r.id)));
