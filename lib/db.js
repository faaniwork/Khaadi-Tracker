...
export async function getMyRole(email) {
  ...
  if (!rows.length) {
    // First time this address has ever been seen. Used to just hand back
    // 'viewer' on the fly and forget it ever happened - which meant Access
    // could never list someone who had only ever signed in and been
    // defaulted, and there was no record anywhere of WHEN a new person
    // actually joined. Writing the row now fixes both: they show up on
    // Access like anyone else, and created_at is what the "N new" badge
    // there counts against (see getNewJoinCount). ON CONFLICT DO NOTHING
    // guards two requests from the same brand-new address racing each other.
    try {
      await d1(
        "INSERT INTO access (email, role, notes, created_at) VALUES (?, 'viewer', '', ?) ON CONFLICT(email) DO NOTHING",
        [norm, Date.now()]
      );
    } catch (e) {
      // migration/013_access_join_tracking.sql (the created_at column) not
      // applied to this database yet - every sign-in would otherwise 500
      // the whole board until someone runs it. Falls back to the old
      // behaviour (a role computed on the fly, nothing written down) rather
      // than taking the app down over a column that doesn't exist yet.
...
/**
 * How many people have joined (been auto-registered as a viewer, or added
 * by an admin) since THIS admin last opened the Access page - see
 * markAccessViewed. Each admin gets their own count, not one shared
 * "unread" that one admin clearing it would hide from everyone else.
 */
export async function getNewJoinCount(adminEmail) {
  const norm = String(adminEmail || '').trim().toLowerCase();
  if (!norm) return 0;
  // Same reason as the try/catch in getMyRole: admin_meta and access.
  // created_at only exist once migration/013 has been run. Badge just
  // stays at 0 until then instead of taking the whole board down.
  try {
    const [viewed] = await queryAll('SELECT access_viewed_at FROM admin_meta WHERE admin_email = ?', [norm]);
    const since = viewed ? Number(viewed.access_viewed_at) : 0;
    const [row] = await queryAll(
      'SELECT COUNT(*) as n FROM access WHERE created_at > ? AND lower(email) != ?',
      [since, norm]
    );
...
export async function markAccessViewed(adminEmail) {
  const norm = String(adminEmail || '').trim().toLowerCase();
  if (!norm) return;
  try {
    await d1(
      `INSERT INTO admin_meta (admin_email, access_viewed_at) VALUES (?, ?)
       ON CONFLICT(admin_email) DO UPDATE SET access_viewed_at = excluded.access_viewed_at`,
      [norm, Date.now()]
    );
  } catch (e) {
    if (!String(e?.message || '').includes('no such')) throw e;
...
export async function getBoardData(email) {
  ...
  // Only worth a query for the one role that has an Access nav item to put
  // a badge on at all (see navItems in dashboard.jsx).
  const newAccessCount = role === 'admin' ? await getNewJoinCount(email) : 0;
...
export async function getActivityLog(limit = 200, viewerRole = 'admin') {
  ...
  // A profile picture change is not production activity for anyone, admin
  // included - it was never about a batch, a dress, or a decision, just
  // someone's own avatar, and it drowned out entries that actually matter.
  // Creating or revoking a client review link is the same story: a rare,
  // admin-only housekeeping action that only ever confused whoever saw it
  // show up as its own unexplained category in the feed.
  const mapped = rows
    .filter((r) => {
      const scope = String(r.scope || '');
      return !scope.startsWith('profile:') && !scope.startsWith('review-link:');
    })
    .map((r) => ({
      id: r.id,
      at: Number(r.at) || 0,
...
export async function getAccessList(email) {
  ...
  let rows;
  try {
    rows = await queryAll('SELECT email, role, notes, created_at FROM access ORDER BY email ASC');
  } catch (e) {
    if (!String(e?.message || '').includes('no such column')) throw e;
    // migration/013 not applied yet - list still works, just without join
    // dates until it is.
    rows = (await queryAll('SELECT email, role, notes FROM access ORDER BY email ASC')).map((r) => ({
      ...r,
      created_at: 0,
    }));
  }
  return rows.map((r) => ({
    ...
    role: r.role || 'viewer',
    notes: r.notes || '',
    // 0 for every row an admin added by hand before this was tracked -
    // real joins only from migration 013 onward (see its own comment).
...
/**
 * A later reshoot round that already lives as its own sibling "V2"/"V3"
 * folder in Drive, one level up from the dress folders themselves - the
 * layout resyncRelease already recognises but, until now, only ever used to
 * decide which folder is the complete set (see lib/driveSync.js). The
 * others were real images with nowhere the board could point to them.
 * Recorded against the dress they reshot rather than becoming board rows
 * of their own, so "Total Dresses" still counts one garment once.
 */
export async function upsertDressVersionFolder({ dressId, version, folderId, folderName, webViewLink }) {
  await d1(
    `INSERT INTO dress_version_folders (dress_id, version, folder_id, folder_name, web_view_link, updated_at)
     VALUES (?, ?, ?, ?, ?, ?)
     ON CONFLICT(dress_id, version) DO UPDATE SET
       folder_id = excluded.folder_id, folder_name = excluded.folder_name,
       web_view_link = excluded.web_view_link, updated_at = excluded.updated_at`,
    [dressId, version, folderId, folderName || '', webViewLink || '', Date.now()]
  );
}

export async function getDressVersionFolders(dressId) {
  const rows = await queryAll(
    'SELECT version, folder_id, folder_name, web_view_link FROM dress_version_folders WHERE dress_id = ? ORDER BY version ASC',
    [dressId]
  );
  return rows.map((r) => ({
    version: r.version,
...
/**
 * Approved count and "has feedback" count per dress, for the small badge on
 * a dress's own cover tile (collection mosaics, and the dress cards inside
 * a collection). Both file_reviews and file_comments are keyed by dress_id
 * alone, not by which round/version folder a file happens to sit in - a
 * review or comment left on a V2 re-shoot is stored under the same dress_id
 * as everything in the original round - so these counts are already the
 * dress's total across every version without this having to know that any
 * versioning exists at all.
 *
 * Two separate grouped queries rather than one join: a file can have
 * comments with no review row at all (commented on, never approved or
 * rejected), which a join through file_reviews would simply drop.
 */
export async function getReviewCountsForDresses(dressIds) {
  const ids = [...new Set((dressIds || []).map(String))].filter(Boolean);
  const out = {};
  ids.forEach((id) => {
    out[id] = { approved: 0, feedback: 0 };
  });
  if (!ids.length) return out;

  const placeholders = ids.map(() => '?').join(',');
  const [approvedRows, commentedRows] = await Promise.all([
    queryAll(
      `SELECT dress_id, COUNT(*) as n FROM file_reviews
       WHERE dress_id IN (${placeholders}) AND review_status = 'approved'
       GROUP BY dress_id`,
      ids
    ),
    queryAll(
      `SELECT dress_id, COUNT(DISTINCT file_id) as n FROM file_comments
  ...
  approvedRows.forEach((r) => {
    const key = String(r.dress_id);
    if (out[key]) out[key].approved = Number(r.n) || 0;
  });
  commentedRows.forEach((r) => {
    const key = String(r.dress_id);
    if (out[key]) out[key].feedback = Number(r.n) || 0;
  });
  return out;
}
...
/**
 * Nudges a dress row's file count by `delta` (+1 on an upload, -1 on a
 * trash, +1 on restoring one from the bin), rather than waiting for the
 * next "Resync from Drive" to recount from scratch.
 *
 * This is what dresses.files actually was before: a number nobody kept
 * current except a resync job, so a board left open through a live upload
 * showed "0 files" under a folder that plainly had pictures in it, and a
 * client's view of the same dress (which has no resync button at all) had
 * no way to ever catch up. Clamped at 0 so a stale/negative count from
 * whatever inconsistency preceded this can't compound itself.
 */
export async function adjustDressFileCount(dressId, delta) {
  if (!dressId || !delta) return;
  await d1('UPDATE dresses SET files = MAX(0, COALESCE(files, 0) + ?) WHERE id = ?', [delta, dressId]);
}

/**
 * Drops the approve/reject verdict on a file, without touching its comment
 * thread.
 *
 * Called when a file's CONTENTS are replaced by a re-upload (see the
 * replaceFileId path in app/api/drive/upload-session/route.js). The verdict
 * described pixels that no longer exist, so carrying it over would leave a
 * brand new picture wearing an "approved" badge nobody ever gave it. The
 * comments stay: those are a conversation people wrote, often the very
 * reason the image was redone, and deleting them to tidy up a state change
 * would be throwing away the feedback along with the file it was about.
 */
export async function clearFileReview(fileId, by) {
  if (!fileId) return;
  const existing = await queryAll(
    'SELECT review_status, file_name, dress_id FROM file_reviews WHERE file_id = ?',
    [fileId]
  );
  if (!existing.length) return;
  const row = existing[0];
  await d1('DELETE FROM file_reviews WHERE file_id = ?', [fileId]);
  await logChange(
    by || 'Someone',
    `file:${row.dress_id}`,
    row.file_name || fileId,
