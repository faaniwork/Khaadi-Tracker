...
export async function getProfiles() {
  try {
    const rows = await queryAll('SELECT email, display_name, avatar, updated_at FROM profiles');
    const byEmail = {};
    const byName = {};
    rows.forEach((r) => {
      const entry = {
      ...
      // The log table records an author's NAME, not their email, so Activity
      // needs a name lookup to put a face against an entry.
...
export async function upsertProfile({ email, displayName, avatar }) {
  ...
  await d1(
    ...
    [norm, String(displayName || '').trim(), value, now]
  );
  await logChange(displayName || norm, 'profile:' + norm, 'picture', '', value ? 'updated' : 'removed');
  return { email: norm, name: String(displayName || '').trim(), avatar: value, updatedAt: now };
}
...
export async function setFileReview({
  ...
  await logChange(by || 'Client', `file:${dressId}`, fileName || fileId, oldStatus, label);
