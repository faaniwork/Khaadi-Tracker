...
export async function listChatMessages(limit = 200) {
  const rows = await queryAll(
    'SELECT id, email, name, text, at FROM chat_messages ORDER BY id DESC LIMIT ?',
...
export async function postChatMessage({ email, name, text }) {
  ...
  const at = Date.now();
  const result = await d1('INSERT INTO chat_messages (email, name, text, at) VALUES (?, ?, ?, ?)', [
    email,
    name || '',
    clean,
    at,
  ...
  return { id, email, name: name || '', text: clean, at };
}
