...
/**
 * The style code most collections are actually named by - "1-26-407",
 * "2-26-405", "B1- 1-26-414" all end in the number that's stitched into
 * the garment itself, which is the number that means something on a rack
 * or a PO, unlike "Sep 28 Release / 1-26-407" as a whole. Read as the LAST
 * run of digits in the name rather than the first, since several of these
 * names lead with an unrelated one ("1-26-407": the "1" is a season/batch
 * marker, "407" is the actual style code).
 */
export function collectionCodeFromName(name) {
  // The LAST whole token, not just the last run of digits: "R-1E4" is style
  // code "1E4" (a letter mixed into the digits, not a pure number), which a
  // digits-only match would have cut down to a bare "4". Split on anything
  // that isn't a letter or digit, so "1-26-407", "B1- 1-26-414" and
  // "Khaadi 304" all still land on their trailing number too.
  const tokens = String(name || '').split(/[^A-Za-z0-9]+/).filter(Boolean);
  const last = tokens[tokens.length - 1] || '';
  // Only counts as a code if it actually has a digit in it - a collection
  // with no number anywhere ("Flats") should fall back to no prefix, not
  // have its whole name mistaken for a style code.
  return /\d/.test(last) ? last : '';
}

/**
 * The short, sortable filename every uploaded image gets: which collection,
 * which dress, which revision round, which picture in that round. A folder
 * full of camera-export names ("IMG_4821.jpg", "affan_khan_..._9182.jpg")
 * tells nobody anything; "407-D3-V2-P7.jpg" says it on sight - the style
 * code first because that's the number someone actually searches a folder
 * by. Extension is kept as uploaded, everything else is derived, never
 * typed by hand.
 *
 * The code prefix is omitted only when the collection's own name has no
 * trailing number to read one from - an old or hand-named collection
 * rather than something broken.
 */
export function smartImageName({ dressName, collectionName, version, position, ext, suffix }) {
  const d = dressNumberFromName(dressName);
  const code = collectionCodeFromName(collectionName);
  const cleanExt = ext ? `.${String(ext).replace(/^\./, '')}` : '';
  const cleanSuffix = suffix ? `-${suffix}` : '';
  const codePrefix = code ? `${code}-` : '';
  return `${codePrefix}D${d ?? 0}-V${version || 1}-P${position}${cleanSuffix}${cleanExt}`;
}
