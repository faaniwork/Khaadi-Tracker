 ...
 * Only the most recently modified container's dresses count.
 *
 * Once a collection has ANY version container, it wins outright — any
 * dress-looking folders sitting directly in the collection are treated as
 * an earlier, pre-versioning cut of the same garments, not additional
 * dresses, and are reported as superseded rather than counted alongside
 * the container's. A batch that gets reshot usually leaves its original
 * flat folders in place rather than tidying them into a "V1" of their own,
 * and counting both was exactly what let one physical dress appear on the
 * board twice, once under its old name and once under its new one.
 *
...
export async function resyncRelease({ email, release, by, rootFolderId }) {
  ...
  const perCollection = await mapWithConcurrency(collectionFolders, CONCURRENCY, async (col) => {
    ...
    if (containers.length > 0) {
      // Most recently modified container wins outright — including over any
      // dress-looking folders sitting directly in the collection, which are
      // an earlier cut of the same dresses, not extra ones.
      ...
      superseded = [
        ...
        ...directDresses.map((f) => `${col.name} / ${f.name} (superseded by ${winner.folder.name})`),
      ];
    } else {
