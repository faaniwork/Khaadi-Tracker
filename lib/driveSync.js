...
import { applyDriveResync, getReleaseFolderId, getReleaseFolderMap, upsertDressVersionFolder } from '@/lib/db';
 ...
 */
async function normalizeRound({ folderId, dressName, collectionName, version, suffix }) {
  ...
  await mapWithConcurrency(images, CONCURRENCY, async (file, i) => {
    ...
    const wanted = smartImageName({ dressName, collectionName, version, position: i + 1, ext, suffix });
 ...
 *
 * `versionOverride` is for a folder whose own name can't say what round it
 * is - a collection-level sibling "V2"/"V3" wrapper's dress folder is still
 * just named "Dress 1", the round comes from its PARENT instead (see the
 * later-cut handling in resyncRelease below).
 *
 * Also reaches into the dress's own "Rejected" subfolder (see
 * REJECTED_FOLDER_NAME in lib/drive.js), which this used to skip entirely -
 * confirmed on real data as nearly two-thirds of one dress's images left
 * with their original camera-export names, since a rejected image is still
 * real, still that dress and round, just not the pick. Named the same way
 * with a "-rejected" suffix so it never collides with the picks' own
 * numbering in the parent folder. Any OTHER subfolder (an "Approved" folder
 * some dresses already had sitting in Drive, for instance) is left alone -
 * that is somebody's own manual organising, not something this owns.
 */
async function normalizeDressFileNames({ dressFolderId, dressName, collectionName, versionOverride }) {
  const selfVersion = DRESS_VERSION_PATTERN.exec(String(dressName || '').trim());
  const version = versionOverride ?? (selfVersion ? Number(selfVersion[1]) : 1);
  ...
  const rejectedFolder = children.find((f) => f.isFolder && f.name === REJECTED_FOLDER_NAME);
  await normalizeRound({ folderId: dressFolderId, dressName, collectionName, version });
  if (rejectedFolder) {
    await normalizeRound({ folderId: rejectedFolder.id, dressName, collectionName, version, suffix: 'rejected' });
  }
...
export async function resyncRelease({ email, release, by, rootFolderId }) {
  ...
  const perCollection = await mapWithConcurrency(collectionFolders, CONCURRENCY, async (col) => {
    ...
    if (directDresses.length > 0) {
      ...
      if (containers.length > 0) {
        ...
        // A later cut is never a board row of its own (see ONE GARMENT, ONE
        // ROW above), but it is still real images someone shot, with
        // nowhere for the board to point at them until now. Matched back to
        // the winner's dress by name - same "Dress 1" in V1 and in V2 is
        // the same garment, two rounds - and recorded so the images stay
        // reachable and get the same D#-V#-P# naming as everything else.
        // An unmatched later dress (reshot under a name that changed, or
        // added new in V2) has nothing to attach to and is left alone.
        const byName = new Map(winner.dresses.map((f) => [f.name.trim().toLowerCase(), f]));
        laterVersions = rest.flatMap((c) => {
          const v = versionOf(c.folder.name);
          if (!Number.isFinite(v)) return [];
          return c.dresses.flatMap((f) => {
            const match = byName.get(f.name.trim().toLowerCase());
            return match ? [{ winnerDressId: match.id, version: v, folder: f, collection: col.name }] : [];
          });
        });
      } else {
        dresses = [];
  ...
  await mapWithConcurrency(dressFolders, CONCURRENCY, async (d) => {
    try {
      await normalizeDressFileNames({ dressFolderId: d.id, dressName: d.dress, collectionName: d.collection });
    } catch {
      // A rename failing here never touched the resync result above.
    }
  });

  // Later cuts matched back to a board dress above: named the same way as
  // everything else, and recorded so the dress's own file panel can point
  // at them (see getDressVersionFolders / dress_version_folders) even
  // though they never become rows of their own.
  await mapWithConcurrency(laterVersionLinks, CONCURRENCY, async (lv) => {
    try {
      await normalizeDressFileNames({
        dressFolderId: lv.folder.id,
        dressName: lv.folder.name,
        collectionName: lv.collection,
        versionOverride: lv.version,
      });
      await upsertDressVersionFolder({
        dressId: lv.winnerDressId,
        version: lv.version,
        folderId: lv.folder.id,
        folderName: lv.folder.name,
        webViewLink: lv.folder.webViewLink,
      });
    } catch {
      // Best-effort, same as the rename loop above.
    }
  });
