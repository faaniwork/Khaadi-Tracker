 ...
 */
async function normalizeRound({ folderId, dressName, version, suffix }) {
  ...
  await mapWithConcurrency(images, CONCURRENCY, async (file, i) => {
    ...
    const wanted = smartImageName({ dressName, version, position: i + 1, ext, suffix });
 ...
 */
async function normalizeDressFileNames({ dressFolderId, dressName, versionOverride }) {
  ...
  await normalizeRound({ folderId: dressFolderId, dressName, version });
  if (rejectedFolder) {
    await normalizeRound({ folderId: rejectedFolder.id, dressName, version, suffix: 'rejected' });
  }
      ...
      if (containers.length > 0) {
        ...
        laterVersions = rest.flatMap((c) => {
          ...
          return c.dresses.flatMap((f) => {
            ...
            return match ? [{ winnerDressId: match.id, version: v, folder: f }] : [];
          });
        });
      } else {
        dresses = [];
