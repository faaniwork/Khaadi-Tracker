...
export function driveThumbUrl({ fileId, dressId, token }) {
  const qs = new URLSearchParams({ fileId, dressId });
  // The <img> tag cannot send headers, so the token has to ride in the query
  // string for previews specifically.
