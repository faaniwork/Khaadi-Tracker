...
async function postJSON(url, body) {
  ...
  if (!res.ok) {
    ...
    // Carried through so the UI can tell "your Drive sign-in expired" apart
    // from any other failure and offer the actual fix (sign out, sign back
    // in) instead of a generic error toast - see DRIVE_ACCESS_REQUIRED in
    // lib/googleUserToken.js.
...
export function markAccessViewed() {
  return postJSON("/api/access/mark-viewed", {});
}
...
async function readError(res) {
  ...
  const err = new Error((data && data.error) || `Request failed (${res.status})`);
  err.status = res.status;
  // See the matching comment on postJSON above.
  if (data && data.code) err.code = data.code;
  return err;
}

async function requestJSON(url, { method = "GET", body, auth, signal } = {}) {
  const res = await fetch(url, {
    method,
    cache: "no-store",
...
/**
 * Approved / has-feedback counts for a batch of dresses at once, for the
 * small badge on their cover tiles - see app/api/drive/review-counts for
 * why this is its own lightweight route instead of one driveList call per
 * dress.
 */
export function fetchReviewCounts({ dressIds, auth }) {
  return requestJSON("/api/drive/review-counts", { method: "POST", body: { dressIds }, auth });
}
 ...
 */
/**
 * Tells the board a new file actually landed - see app/api/drive/upload-
 * complete/route.js for why this has to be a separate call rather than
 * something the upload session itself can do.
 */
export function confirmUploadComplete({ dressId }) {
  return requestJSON("/api/drive/upload-complete", { method: "POST", body: { dressId } });
}

export function driveThumbUrl({ fileId, dressId, size, v }) {
  const qs = new URLSearchParams({ fileId, dressId });
  if (size) qs.set("size", String(size));
  // Replacing a picture keeps the same Drive file id on purpose (see
  // initResumableUpdate), so without something in the URL that changes with
  // the contents, every cache between here and Drive would keep serving the
  // old image and "Replace" would look like it had done nothing.
...
export function driveRemovedFiles({ dressId }) {
  return requestJSON(`/api/drive/restore?dressId=${encodeURIComponent(dressId)}`);
}

export function driveRestoreFile({ dressId, fileId }) {
  return requestJSON("/api/drive/restore", { method: "POST", body: { dressId, fileId } });
}
...
/** Admin only. Archives the batch's dresses; optionally trashes its Drive folder. */
export function deleteBatch({ release, trashFolder }) {
  return requestJSON("/api/batches", { method: "DELETE", body: { release, trashFolder } });
}

/** Checks every batch still exists in Drive, removing the ones that do not. */
export function verifyBatches() {
  return requestJSON("/api/drive/verify", { method: "POST", body: {} });
}
 ...
 * looks broken.
 */
export async function driveUpload({ dressId, folderId, file, onProgress, replaceFileId, signal }) {
  if (signal?.aborted) throw new DOMException("Upload cancelled", "AbortError");
  const { uploadUrl, replaced } = await requestJSON("/api/drive/upload-session", {
    method: "POST",
    body: { dressId, folderId, name: file.name, mimeType: file.type, replaceFileId },
    signal,
  });
  ...
  return new Promise((resolve, reject) => {
    ...
    // Only an explicit cancel aborts an upload in flight. Closing this dress
    // or walking back to the board deliberately does NOT - see the upload
    // manager in components/upload-manager.jsx for why the transfer outlives
    ...
    xhr.onload = () => {
      ...
      if (xhr.status >= 200 && xhr.status < 300 && data) {
        return ok({ file: shapeUploadedFile(data), replaced: Boolean(replaced) });
      }
...
export function saveProfile({ avatar, name }) {
  return requestJSON("/api/profile", { method: "POST", body: { avatar, name } });
}

// ---------- team/client chat ----------

export function fetchChatStatus() {
  return requestJSON("/api/chat/status");
}

export function requestChatAccess() {
  return requestJSON("/api/chat/access", { method: "POST" });
}
...
export function sendChatMessage({ text, imageId, imageName, replyToId }) {
  return requestJSON("/api/chat/messages", { method: "POST", body: { text, imageId, imageName, replyToId } });
}

export function deleteChatMessage({ id }) {
  return requestJSON("/api/chat/messages", { method: "DELETE", body: { id } });
}

export function reactToChatMessage({ messageId, emoji }) {
  return requestJSON("/api/chat/react", { method: "POST", body: { messageId, emoji } });
}

export function grantChatAccess({ targetEmail }) {
  return requestJSON("/api/chat/access/grant", { method: "POST", body: { targetEmail } });
}

export function chatImageUrl({ fileId, size }) {
  const qs = new URLSearchParams({ fileId });
  if (size) qs.set("size", size);
  return `/api/chat/thumb?${qs}`;
}

/**
 * Uploads one image for the chat: same two-step dance as driveUpload (the
 * server only hands back a Drive resumable-upload URL, bytes go straight
 * from this browser to Google), scoped to the chat's own "Chat uploads"
 * folder instead of a dress.
 */
export function uploadChatImage({ file, onProgress }) {
  return requestJSON("/api/chat/upload-session", {
    method: "POST",
    body: { name: file.name, mimeType: file.type },
  }).then(
    ({ uploadUrl }) =>
      new Promise((resolve, reject) => {
        ...
        xhr.onload = () => {
          ...
          if (xhr.status >= 200 && xhr.status < 300 && data) {
            return resolve({ id: data.id, name: data.name || file.name });
          }
