...
import { DRESS_VERSION_PATTERN } from './constants';
...
// originalFilename is the name the file had on the uploader's own machine.
// Every upload gets renamed on the way in (see smartImageName), so this is
// the ONLY record of what was actually dropped - and therefore the only way
// to tell that the same picture is being uploaded a second time.
const FILE_FIELDS =
  'id, name, originalFilename, mimeType, size, modifiedTime, createdTime, webViewLink, iconLink, thumbnailLink, hasThumbnail, trashed, parents';
...
/**
 * A Drive client acting as the signed-in person rather than as the service
 * account, for the calls where it matters WHO ends up owning what is created.
 *
 * A folder the service account creates is owned by the service account, which
 * means the team cannot rename, move or trash it from Drive themselves — and
 * this shoot gets reorganised in Drive constantly. Creating it as the person
 * who asked for it leaves them a folder that behaves like any other folder
 * they made by hand.
 */
function driveAs(accessToken) {
  if (!accessToken) return drive();
...
function shapeFile(f) {
  return {
    id: f.id,
    name: f.name || '',
    // Drive falls back to the metadata name when an upload never declared one,
    // so for anything uploaded before this was recorded it simply equals
    // `name` - which matches no dropped filename, so duplicate detection
    // degrades to "no duplicates known" rather than to false positives.
...
// Junk the operating system leaves behind in a shared folder. None of it is
// content, and .DS_Store in particular turns up in every folder anyone has
// opened on a Mac.
const OS_JUNK = /^(\.DS_Store|\._.*|\.Spotlight-V100|\.Trashes|Thumbs\.db|desktop\.ini)$/i;

export function isOsJunk(name) {
  return OS_JUNK.test(String(name || '').trim());
}
 ...
 *
 * Operating-system junk is dropped here rather than in each caller, since
 * this is the one place every listing passes through: the grid, the
 * lightbox, downloads, the bin and the file counts a resync writes all
 * inherit it, so a dress can never report 21 files when one of them is a
 * .DS_Store.
 */
  ...
  try {
    do {
      ...
      (res.data.files || []).forEach((f) => {
        if (isOsJunk(f.name)) return;
        out.push(shapeFile(f));
      });
...
/**
 * Just the folder ids a file sits in.
 *
 * shapeFile deliberately drops `parents` - the client has no business
 * knowing the shape of the Drive tree above what it is looking at - but the
 * server needs them to check that a file someone claims to be replacing is
 * actually inside the dress they are allowed to write to.
 */
export async function getFileParents(fileId) {
  assertDriveId(fileId, 'file id');
  try {
    const res = await drive().files.get({ fileId, fields: 'id, parents, mimeType', ...SHARED });
    return {
  ...
  return [
    ...main
      .filter((f) => {
        ...
        // Rejected/Removed are merged in above as files, never shown as
        // their own folder tile; a revision round ("V2", "V3") gets its
        // own tab in the files panel instead of a grid tile. Anything
        // else - "Approved", "Edit", whatever else someone made by hand
        // in Drive outside this app - is exactly the "mystery folder tile
        // sitting next to the images" this dress view already decided
        // not to show for version folders; the same call applies to any
        // folder this app doesn't itself understand the shape of.
        if (f.name === REJECTED_FOLDER_NAME || f.name === REMOVED_FOLDER_NAME) return false;
        return DRESS_VERSION_PATTERN.test(f.name);
      })
...
/**
 * Renames a file in place. Plain edit access is enough for this (unlike
 * trashFile, which needs ownership in a personal My Drive), so it always runs
...
export async function initResumableUpload({
  ...
  const res = await fetch(`https://www.googleapis.com/upload/drive/v3/files?${qs}`, {
    ...
    // originalFilename is set explicitly because the browser's PUT sends raw
    // bytes with no filename attached, so Drive would otherwise just echo
    // `name` into it. Recording what the file was actually called is what
    // makes "you already uploaded this one" answerable later, given that
    // `name` is always rewritten to D<n>-V<n>-P<n> on the way in.
    body: JSON.stringify({
      name,
      parents: [folderId],
      ...(originalFilename ? { originalFilename: String(originalFilename).slice(0, 255) } : {}),
    }),
  });
  if (!res.ok) {
    const body = await res.text().catch(() => '');
    // Named for the identity that ACTUALLY made this call. wrapDriveError
    // blames the service account, which is right for every other call in
    // this file but wrong here: an upload runs as the signed-in person, so a
    // 404 means THEIR Google account cannot see the folder, and telling them
    // to check the service account's sharing sends them somewhere useless.
    if (userToken && (res.status === 404 || res.status === 403)) {
      const err = new Error(
        'NO_DRIVE_ACCESS: Your Google account cannot write to this folder in Drive. ' +
          'Being an editor on this board is not enough on its own: the upload is made as you, ' +
          'so the shoot folder has to be shared with your Google account too. Ask an admin to share it.'
      );
      err.code = 'NO_DRIVE_ACCESS';
      err.status = 403;
      throw err;
    }
...
/**
 * A resumable session that overwrites an EXISTING file's contents, keeping
 * its id, its name and its place in the folder.
 *
 * This is what "Replace" means when someone re-uploads a picture that is
 * already here. The alternative - trash the old file and upload a new one -
 * would renumber it (names carry their position, see smartImageName), so
 * replacing picture 3 of 20 would quietly move it to the end of the set. An
 * in-place content update keeps D2-V1-P3 being picture 3.
 *
 * Note this is a PATCH against /files/{id} rather than a POST to /files, and
 * it carries no `parents` - a resumable update that tries to set parents is
 * rejected outright by Drive.
 */
export async function initResumableUpdate({
  fileId,
  originalFilename,
  mimeType,
  origin,
  ...
  assertDriveId(fileId, 'file id');
  if (addParents) assertDriveId(addParents, 'folder id');
  if (removeParents) assertDriveId(removeParents, 'folder id');
  let accessToken = userToken;
  if (!accessToken) {
    const token = await getAuth().getAccessToken();
    accessToken = typeof token === 'string' ? token : token?.token;
  ...
  // Reparenting is a query parameter on this endpoint, not a metadata field -
  // a resumable update that puts `parents` in the body is rejected outright.
  if (addParents) qs.set('addParents', addParents);
  if (removeParents) qs.set('removeParents', removeParents);
  const res = await fetch(`https://www.googleapis.com/upload/drive/v3/files/${fileId}?${qs}`, {
    method: 'PATCH',
    headers: {
      Authorization: `Bearer ${accessToken}`,
      'Content-Type': 'application/json; charset=UTF-8',
      'X-Upload-Content-Type': mimeType || 'application/octet-stream',
      // Same CORS rule as initResumableUpload: the allowed origin is fixed
      // when the session is created, not when the browser PUTs to it.
      ...(origin ? { Origin: origin } : {}),
    },
    body: JSON.stringify({
      ...(originalFilename ? { originalFilename: String(originalFilename).slice(0, 255) } : {}),
  ...
  if (!res.ok) {
    ...
    if (userToken && (res.status === 404 || res.status === 403)) {
      const err = new Error(
        'NO_DRIVE_ACCESS: Your Google account cannot replace that file in Drive. ' +
          'The upload is made as you, so the shoot folder has to be shared with your ' +
          'Google account with edit rights. Ask an admin to share it.'
      );
      err.code = 'NO_DRIVE_ACCESS';
      err.status = 403;
      throw err;
    }
    wrapDriveError({ code: res.status, message: body.slice(0, 300) }, `file ${fileId}`);
  }
  const uploadUrl = res.headers.get('location');
  if (!uploadUrl) throw new Error('Drive did not return an upload session for that file.');
  return uploadUrl;
}
 ...
 */
/**
 * Whether a folder is still there: 'live', 'trashed', 'missing', or 'unknown'.
 *
 * One call, and it never throws. 'unknown' is the important one: Drive
 * answers 404 both for a folder that was deleted and for one this account can
 * no longer see, but a rate limit or a network failure is neither, and
 * treating those as "gone" would take a live batch off the board. Callers act
 * only on a definite answer.
 */
export async function getFolderState(folderId) {
  try {
    assertDriveId(folderId, 'folder id');
    const res = await drive().files.get({ fileId: folderId, fields: 'id, trashed', ...SHARED });
    return res.data.trashed ? 'trashed' : 'live';
  } catch (e) {
    const status = e?.code || e?.response?.status;
    if (status === 404) return 'missing';
    console.error('folder state check failed', folderId, status || e?.message);
    return 'unknown';
  }
}

/**
 * Gives a Google account edit access to a folder, and everything under it.
 *
 * This is what makes "editor on the board" mean something in Drive. An upload
 * runs as the signed-in person, so Drive checks THEIR account against the
 * folder: without this, being an editor here got you a permission error from
 * Google the moment you tried to add a file.
 *
 * Notifications are off. Drive emails the person by default, and being added
 * to a shoot folder is not news worth a mail every time somebody's role is
 * adjusted.
 *
 * Requires the service account to be allowed to share, which an owner can
 * switch off. The caller is told when that is why it failed, rather than
 * being left to guess.
 */
export async function grantFolderAccess({ folderId, email, role = 'writer' }) {
  assertDriveId(folderId, 'folder id');
  const address = String(email || '').trim().toLowerCase();
  if (!address) throw new Error('An email is required');
  try {
    const res = await drive().permissions.create({
      fileId: folderId,
      sendNotificationEmail: false,
      requestBody: { type: 'user', role, emailAddress: address },
      fields: 'id, role, emailAddress',
      ...SHARED,
    });
    return { id: res.data.id, role: res.data.role, email: res.data.emailAddress };
  } catch (e) {
    const status = e?.code || e?.response?.status;
    if (status === 403) {
      const err = new Error(
        'DRIVE_CANNOT_SHARE: Drive would not let this app share the shoot folder. ' +
          "Its owner has to allow editors to change permissions, or share the folder with them by hand."
      );
      err.code = 'DRIVE_CANNOT_SHARE';
      err.status = 403;
      throw err;
    }
    wrapDriveError(e, `folder ${folderId}`);
...
/**
 * Takes that access away again, so losing the board does not leave someone
 * with the Drive folder.
 *
 * Only ever removes a permission granted to that exact address, and never an
 * owner's: deleting an ownership record is not something a role change on
 * this board should be able to do.
 */
export async function revokeFolderAccess({ folderId, email }) {
  assertDriveId(folderId, 'folder id');
  const address = String(email || '').trim().toLowerCase();
  ...
  try {
    ...
    const mine = (res.data.permissions || []).filter(
      (p) =>
        p.type === 'user' &&
        p.role !== 'owner' &&
        String(p.emailAddress || '').toLowerCase() === address
    );
    for (const p of mine) {
      await drive().permissions.delete({ fileId: folderId, permissionId: p.id, ...SHARED });
    }
    return { removed: mine.length };
  } catch (e) {
    const status = e?.code || e?.response?.status;
    // Not being allowed to manage sharing is not a reason to fail a role
    // change on the board; the caller reports it instead.
    if (status === 403) return { removed: 0, refused: true };
    wrapDriveError(e, `folder ${folderId}`);
  }
}
...
export async function createFolder({ parentId, name, accessToken }) {
  assertDriveId(parentId, 'folder id');
  const trimmed = String(name || '').trim();
  if (!trimmed) throw new Error('Folder name cannot be empty');
  try {
    // `accessToken` is the caller's own Google token where one is available,
    // so the folder is owned by them rather than by the service account. See
    // driveAs for why that matters.
    const res = await driveAs(accessToken).files.create({
...
export async function trashFile(fileId, accessToken) {
  assertDriveId(fileId, 'file id');
  try {
    // As the caller where a token is supplied: in this Drive only a file's
    // OWNER may trash it, and anything this app created was created as the
    // person who asked for it. The service account cannot trash their folder.
    const res = await driveAs(accessToken).files.update({
      fileId,
...
export async function dressScopeParents(dressFolderId) {
  ...
  // Rejected AND Removed both count as inside the dress. Removed has to be
  // here for the bin to show thumbnails of what it holds — without it the
  // preview route treats a removed file as living outside the dress and
  // answers 404, so the bin could only ever list filenames.
  const [rejected, removed] = await Promise.all([
    findSubfolders({ parentId: dressFolderId, name: REJECTED_FOLDER_NAME }),
    findSubfolders({ parentId: dressFolderId, name: REMOVED_FOLDER_NAME }),
  ]);
  return [dressFolderId, ...rejected.map((f) => f.id), ...removed.map((f) => f.id)];
}
...
/**
 * What is sitting in this dress's "Removed" subfolder — the files taken off
 * the board that Drive would not let us trash. Empty list when the folder was
 * never created, which is the normal case.
 */
export async function listRemovedFiles(dressFolderId) {
  assertDriveId(dressFolderId, 'folder id');
  const folders = await findSubfolders({ parentId: dressFolderId, name: REMOVED_FOLDER_NAME });
  if (!folders.length) return [];
  const lists = await Promise.all(folders.map((f) => listFolder(f.id)));
  return lists.flat().filter((f) => !f.isFolder);
}

/**
 * Puts a removed file back where it came from.
 *
 * The dress folder is the only place it can go: "Removed" is a flat parking
 * bay directly inside the dress, so there is no earlier location to restore
 * to and nothing to remember. If Drive DID let us trash the file, this
 * untrashes it instead.
 */
export async function restoreRemovedFile({ fileId, dressFolderId }) {
  assertDriveId(fileId, 'file id');
  assertDriveId(dressFolderId, 'folder id');
  try {
    const res = await drive().files.get({ fileId, fields: 'id, name, trashed', ...SHARED });
    if (res.data.trashed) {
      await drive().files.update({ fileId, requestBody: { trashed: false }, fields: 'id', ...SHARED });
    }
  } catch (e) {
    wrapDriveError(e, `file ${fileId}`);
  }
  await moveBetween({ fileId, addParent: dressFolderId });
  return { id: fileId, restoredTo: dressFolderId };
}
